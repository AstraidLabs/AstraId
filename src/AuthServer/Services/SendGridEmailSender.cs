...
public sealed class SendGridEmailSender : IEmailSender
{
    ...
    public SendGridEmailSender(HttpClient httpClient, IOptions<EmailOptions> options, ILogger<SendGridEmailSender> logger)
    {
    ...
    public async Task SendAsync(EmailMessage message, CancellationToken cancellationToken)
    {
        ...
        var payload = new
        {
            ...
            personalizations = new[] { new { to = new[] { new { email = message.ToEmail, name = message.ToName } }, subject = message.Subject } },
        ...
        {
            _logger.LogWarning("Email provider {Provider} returned non-success status {StatusCode}.", "SendGrid", (int)response.StatusCode);
            response.EnsureSuccessStatusCode();
        }

        var messageId = response.Headers.TryGetValues("X-Message-Id", out var values) ? values.FirstOrDefault() : null;
        _logger.LogInformation("Email sent via provider {Provider}. StatusCode: {StatusCode}. MessageId: {MessageId}", "SendGrid", (int)response.StatusCode, string.IsNullOrWhiteSpace(messageId) ? "n/a" : messageId);
    }
    ...
    private async Task<HttpResponseMessage> SendWithRetryAsync(Func<Task<HttpResponseMessage>> operation, CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 1; attempt <= 3; attempt++)
        {
            ...
            if (!IsTransientFailure(response.StatusCode) || attempt == 3)
            {
                return response;
            }
    ...
    private static object[] BuildContent(EmailMessage message)
    {
        ...
        if (!string.IsNullOrWhiteSpace(message.TextBody)) content.Add(new { type = "text/plain", value = message.TextBody });
        if (!string.IsNullOrWhiteSpace(message.HtmlBody)) content.Add(new { type = "text/html", value = message.HtmlBody });
