     ...
     .ToList();
        ...
        foreach (var entry in ordered)
        {
            ...
            if (entry.Status is SigningKeyStatus.Previous)
            {
                ...
                {
                    // If we cannot unprotect the previous private material, we can't add it as a credential.
                    // This is OK: rollover validation still works if clients already cached the old key
                    // BUT ideally you want to keep previous key material readable during grace period.
                    logger.LogWarning(ex, "Failed to unprotect previous signing key material for {Kid}. It won't be published.", entry.Kid);
                }
            }
        }
    }
