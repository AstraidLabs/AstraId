...
import { getLoginContext, register, resolveReturnUrl, type LoginContext } from "../api/authServer";
...
const Register = () => {
  ...
  useEffect(() => {
    ...
    getLoginContext(returnUrl).then((context) => {
      if (mounted) setLoginContext(context);
    }).catch(() => {
      if (mounted) setLoginContext(null);
    });
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit} noValidate>
          ...
          {loginContext?.hasOidcContext && loginContext.appName ? (
            <header className="rounded-lg border border-indigo-500/40 bg-indigo-500/10 p-3" aria-live="polite">
              <h2 className="text-sm font-semibold text-indigo-100">Registering for {loginContext.appName}</h2>
